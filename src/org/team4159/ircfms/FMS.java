...
public class FMS {
	...
	private static class Handler extends Thread {
        ...
        public Handler(Socket socket) {
            this.socket = socket;
            System.out.println("Socket opened from " + socket.getInetAddress());
        }
        ...
        public void run() {
            try {
                ...
                out.println("I_G");
                number = Integer.parseInt(in.readLine().replace("I_", ""));//Lazy way to do the stuff... change later
                synchronized (robots) {
                    for(Robot r : robots)
                    {
                    	if(r.number == number)
                ...
                while (true) {
                    ...
                    if(input.indexOf("HB_LUB") != -1)
                    {
                    	...
                    	System.out.println("Robot " + robot.number + ": " + input); //Print out unknown command
